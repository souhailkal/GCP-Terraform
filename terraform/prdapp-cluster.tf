resource "google_container_cluster" "prdapp" {
  provider    = google.prdapp
  name        = var.prdapp_name
  description = "${var.prdapp_desc} cluster"
  location    = "${var.prdapp_region}-b"
...
resource "google_container_node_pool" "prdapp-frontend" {
  provider           = google.prdapp
  name               = "${var.prdapp_name}-frontend"
  location           = "${var.prdapp_region}-b"
  cluster            = google_container_cluster.prdapp.name
  initial_node_count = 1
...
provider "kubernetes" {
  host             = google_container_cluster.prdapp.endpoint
  alias            = "prdapp"
  load_config_file = false
  username         = google_container_cluster.prdapp.master_auth[0].username
  password         = google_container_cluster.prdapp.master_auth[0].password
  client_certificate = base64decode(
    google_container_cluster.prdapp.master_auth[0].client_certificate,
  )
  client_key = base64decode(google_container_cluster.prdapp.master_auth[0].client_key)
  cluster_ca_certificate = base64decode(
    google_container_cluster.prdapp.master_auth[0].cluster_ca_certificate,
  )
}
